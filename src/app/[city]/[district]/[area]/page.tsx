...
const introTemplates = [
  (district: string, area: string) =>
    `${district} ${area}에서 전체인테리어를 계획할 때는 디자인 이미지뿐 아니라 현재 공간의 구조와 실제 사용하는 사람의 생활 또는 운영 방식을 함께 확인하는 것이 중요합니다. 더세이브인테리어는 공간 전체의 연결성을 기준으로 주거와 상업 인테리어를 계획합니다.`,

  (district: string, area: string) =>
    `${area}의 주거 또는 상업공간은 같은 평수라도 건물 구조와 용도에 따라 필요한 인테리어 방향이 달라질 수 있습니다. ${district} ${area} 현장의 구조와 동선, 사용 목적을 확인한 뒤 전체 공간을 하나의 방향으로 구성합니다.`,

  (district: string, area: string) =>
    `${district} ${area}에서 아파트, 주택, 상가, 매장이나 사무실 전체인테리어를 준비한다면 먼저 공간이 어떻게 사용될 것인지 살펴볼 필요가 있습니다. 전체적인 디자인과 기능, 동선과 공정의 연결을 함께 고려해 계획합니다.`,

  (district: string, area: string) =>
    `${area} 전체인테리어는 단순히 마감재를 바꾸는 작업이 아니라 공간의 기능과 분위기를 새롭게 정리하는 과정입니다. 더세이브인테리어는 ${district} ${area}의 주거 및 상업공간을 대상으로 전체적인 공간 구성을 상담합니다.`,

  (district: string, area: string) =>
    `${district} ${area}의 공간마다 면적과 기존 구조, 채광과 설비 조건이 다르기 때문에 획일적인 방식보다 현장에 맞는 전체인테리어 계획이 필요합니다. 사용하는 목적을 먼저 확인하고 전체적인 시공 방향을 정합니다.`,
];

const residentialTemplates = [
  (area: string) =>
    `${area}의 아파트, 빌라, 주택, 오피스텔 등 주거공간에서는 가족 구성과 생활 패턴을 기준으로 거실과 주방, 침실, 수납공간의 관계를 함께 살펴봅니다. 각각의 공간을 따로 보는 것이 아니라 집 전체가 자연스럽게 연결되도록 계획합니다.`,

  (area: string) =>
    `${area} 주거 전체인테리어는 실제 생활에서 자주 이동하는 동선과 필요한 수납량, 가족별 공간 사용 방식을 확인하는 과정부터 시작합니다. 공간 전체의 분위기와 기능이 균형을 이룰 수 있도록 구성합니다.`,

  (area: string) =>
    `${area} 아파트와 주거공간은 평수만으로 인테리어 방향을 정하기보다 현재 평면과 가족의 생활 방식을 함께 보는 것이 중요합니다. 주방과 거실, 침실과 수납이 서로 자연스럽게 연결되는 전체 공간을 계획합니다.`,

  (area: string) =>
    `${area}의 주거공간에서는 편안함과 실제 사용성이 중요합니다. 거실의 개방감, 주방의 활용도, 침실 구성과 수납까지 하나의 흐름으로 정리해 주거 전체인테리어를 계획합니다.`,
];

const commercialTemplates = [
  (area: string) =>
    `${area}의 상가, 매장, 카페, 음식점, 사무실 등 상업공간은 업종과 운영 방식에 따라 필요한 구조가 달라집니다. 고객의 이동 경로와 직원의 업무 동선, 공간이 전달해야 하는 분위기를 함께 고려합니다.`,

  (area: string) =>
    `${area} 상업 전체인테리어에서는 보기 좋은 디자인과 실제 운영의 편리함을 함께 생각해야 합니다. 입구에서 내부로 이어지는 고객 동선과 작업공간, 좌석 또는 업무공간의 배치를 전체적으로 계획합니다.`,

  (area: string) =>
    `${area}의 매장이나 사무공간은 방문객이 처음 공간을 마주하는 순간부터 실제 이용 과정까지 고려하는 것이 중요합니다. 업종의 특성과 브랜드 분위기, 업무 효율을 반영해 상업공간 전체를 구성합니다.`,

  (area: string) =>
    `${area} 상업공간은 동일한 면적이라도 카페, 음식점, 매장, 사무실 등 업종에 따라 필요한 설비와 공간 배치가 달라집니다. 실제 운영 방식을 확인한 뒤 전체인테리어 방향을 구체화합니다.`,
];
